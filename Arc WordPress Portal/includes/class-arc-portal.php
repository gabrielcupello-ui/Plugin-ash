...
?></th>						<td>							<input type="text" name="<?php echo esc_attr( $this->option_key );
?>[home_title]" value="<?php echo esc_attr( $settings['home_title'] );
?>" class="regular-text">						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Welcome description', 'arc-portal' );
?></th>						<td>							<textarea name="<?php echo esc_attr( $this->option_key );
?>[home_description]" class="regular-text" rows="3"><?php echo esc_textarea( $settings['home_description'] );
?></textarea>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Help email', 'arc-portal' );
?></th>						<td>							<input type="email" name="<?php echo esc_attr( $this->option_key );
?>[help_email]" value="<?php echo esc_attr( $settings['help_email'] );
?>" class="regular-text">						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Logo URL', 'arc-portal' );
?></th>						<td>							<input type="url" name="<?php echo esc_attr( $this->option_key );
?>[logo_url]" value="<?php echo esc_url( $settings['logo_url'] );
?>" class="regular-text">							<p class="description"><?php esc_html_e( 'Leave blank to display the title.', 'arc-portal' );
?></p>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Allowed roles', 'arc-portal' );
?></th>						<td>							<?php foreach ( $roles as $role_key => $role_label ) : ?>								<label style="display:block;margin-bottom:4px;">									<input type="checkbox" name="<?php echo esc_attr( $this->option_key );
?>[allowed_roles][]" value="<?php echo esc_attr( $role_key );
...
?>							<p class="description"><?php esc_html_e( 'Administrators always have access.', 'arc-portal' );
?></p>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Pass WordPress email to the apps', 'arc-portal' );
?></th>						<td>							<label>								<input type="checkbox" name="<?php echo esc_attr( $this->option_key );
?>[pass_email]" value="1" <?php checked( $settings['pass_email'] );
?>>								<?php esc_html_e( 'Append ?wp_user=email to embedded URLs. Each app must accept this parameter.', 'arc-portal' );
?>							</label>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Login redirect URL', 'arc-portal' );
?></th>						<td>							<input type="url" name="<?php echo esc_attr( $this->option_key );
?>[login_redirect]" value="<?php echo esc_url( $settings['login_redirect'] );
?>" class="regular-text">							<p class="description"><?php esc_html_e( 'URL to send unauthenticated users to. Leave blank for wp-login.php.', 'arc-portal' );
?></p>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Logout URL', 'arc-portal' );
?></th>						<td>							<input type="url" name="<?php echo esc_attr( $this->option_key );
?>[logout_url]" value="<?php echo esc_url( $settings['logout_url'] );
?>" class="regular-text">							<p class="description"><?php esc_html_e( 'Custom logout URL. Leave blank for wp_logout_url().', 'arc-portal' );
?></p>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'Portal apps', 'arc-portal' );
?></th>						<td>							<table class="widefat">								<thead>									<tr>										<th><?php esc_html_e( 'Enabled', 'arc-portal' );
?></th>										<th><?php esc_html_e( 'Module', 'arc-portal' );
?></th>										<th><?php esc_html_e( 'Label', 'arc-portal' );
...
?></td>										<td><input type="text" name="<?php echo esc_attr( $this->option_key );
?>[apps][<?php echo esc_attr( $key );
?>][label]" value="<?php echo esc_attr( $app['label'] );
?>" class="regular-text"></td>										<td><input type="url" name="<?php echo esc_attr( $this->option_key );
?>[apps][<?php echo esc_attr( $key );
?>][url]" value="<?php echo esc_url( $app['url'] );
?>" class="regular-text" style="width:100%;"></td>										<td><?php echo esc_html( $status_dot . ' ' . $status['text'] );
?></td>										<td>											<select name="<?php echo esc_attr( $this->option_key );
?>[apps][<?php echo esc_attr( $key );
?>][target]">												<option value="iframe" <?php selected( $app['target'], 'iframe' );
?>><?php esc_html_e( 'Iframe', 'arc-portal' );
?></option>												<option value="new_tab" <?php selected( $app['target'], 'new_tab' );
?>><?php esc_html_e( 'New tab', 'arc-portal' );
?></option>											</select>										</td>									</tr>									<?php endforeach;
?>								</tbody>							</table>							<p class="description"><?php esc_html_e( 'URLs must be the deployment URLs (Deploy > Web app) for each Apps Script project. Paste each app link.', 'arc-portal' );
?></p>						</td>					</tr>					<tr>						<th scope="row"><?php esc_html_e( 'SSO integration with Google Apps Script', 'arc-portal' );
?></th>						<td>							<label for="arc_gas_auth_url"><?php esc_html_e( 'Shared GAS endpoint URL', 'arc-portal' );
?></label><br>							<input type="url" id="arc_gas_auth_url" name="arc_portal_app_gas_auth_url" value="<?php echo esc_url( get_option( 'arc_portal_app_gas_auth_url', '' ) );
?>" class="regular-text" style="margin-top:6px;">							<p class="description">								<?php esc_html_e( 'Optional. URL of an Apps Script Web App that validates credentials/users for all apps. See docs/gas-auth-bridge.md.', 'arc-portal' );
?>							</p>							<label for="arc_gas_api_secret" style="display:block;margin-top:14px;"><?php esc_html_e( 'API Secret to sign tokens', 'arc-portal' );
?></label>							<input type="text" id="arc_gas_api_secret" name="arc_portal_app_gas_api_secret" value="<?php echo esc_attr( get_option( 'arc_portal_app_gas_api_secret', '' ) );
?>" class="regular-text" style="margin-top:6px;">							<p class="description">								<?php esc_html_e( 'Leave blank to use wp_salt(\'auth\'). If set, share it with the Apps Script endpoint too.', 'arc-portal' );
?>							</p>						</td>					</tr>				</table>				<?php submit_button();
...
?></p>					<?php if ( $page_url ) : ?>						<p class="text-sm text-emerald-600 mb-4"><span class="font-semibold">✓</span> <a href="<?php echo esc_url( $page_url );
?>" target="_blank" class="underline"><?php echo esc_html( $page_url );
?></a></p>					<?php else : ?>						<p class="text-sm text-amber-600 mb-4"><span class="font-semibold">⚠</span> <?php esc_html_e( 'No page yet', 'arc-portal' );
?></p>					<?php endif;
?>					<form method="post" action="">						<?php wp_nonce_field( 'arc_portal_create_page' );
...
?>"><?php esc_html_e( 'Edit advanced settings', 'arc-portal' );
?></a>				</div>			</div>			<div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mt-6">				<h2 class="text-xl font-bold mb-2"><?php esc_html_e( 'Shortcode', 'arc-portal' );
?></h2>				<p class="text-sm text-gray-500 mb-3"><?php esc_html_e( 'Use this shortcode on any page:', 'arc-portal' );
?></p>				<code class="bg-gray-100 px-3 py-2 rounded text-sm">[arc_portal]</code>			</div>		</div>		<?php	}
	/**
	 * AJAX handler to render an app inline using a shortcode or custom action.
	 */
